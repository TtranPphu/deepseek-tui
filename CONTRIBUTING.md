...
- End the body with a blank line followed by a signature line:
  - If an agent authored the changes, always sign with the agent harness that ran the session — `OpenCode` for opencode sessions, `Claude` for Claude Code sessions, spelled in the product's own casing — followed by the session's model id (e.g. `OpenCode - deepseek-v4-flash`). The signature's first token names the harness, not an agent role or hierarchy: it is the same whether the work was executed directly or via a delegated subagent, and a subagent never substitutes its own name in place of the harness that ran the session.
